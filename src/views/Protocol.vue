...
import { Plus, Delete, Upload, Download, Rank } from '@element-plus/icons-vue'
...
/* ---- SystemModuleTree 叶子层：模块 → 协议组/接口组 → 项 ---- */
  ...
  return [
    {
      ...
      addLabel: '+协议',
      addType: 'primary',
      items: protos.map((p) => ({ key: `p-${p.id}`, kind: 'protocol', icon: 'Document', label: p.name, ref: p }))
    },
    {
      key: `ig-${module.id}`,
      kind: 'ifGroup',
      ...
      addLabel: '+接口',
      addType: 'success',
      items: ifaces.map((i) => ({ key: `i-${i.id}`, kind: 'interface', icon: 'Link', label: i.name, ref: i }))
    }
  ]
}
...
const DEFAULT_LEAF_NAME = { protoGroup: '新建协议', ifGroup: '新建接口' }
const addProtocolLeaf = (module) => {
  store.addProtocol({ name: DEFAULT_LEAF_NAME.protoGroup, systemId: module.systemId, moduleId: module.id })
  selectedKind.value = 'protocol'
}
const addInterfaceLeaf = (module) => {
  store.addInterface({ name: DEFAULT_LEAF_NAME.ifGroup, systemId: module.systemId, moduleId: module.id })
  selectedKind.value = 'interface'
}
const onTreeAddLeaf = ({ groupKind, module }) => {
  if (groupKind === 'protoGroup') addProtocolLeaf(module)
  else if (groupKind === 'ifGroup') addInterfaceLeaf(module)
}
const onTreeDeleteLeaf = (node) => {
  if (node.kind === 'protocol') store.removeProtocol(node.ref.id)
  else if (node.kind === 'interface') store.removeInterface(node.ref.id)
}
