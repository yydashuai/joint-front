...
const PREVIEWS = {
  standard: {
    tone: 'standard',
    desc: '面向一次发送或接收批次，呈现数据规模、接口明细、异常情况与结论。',
    meta: ['适用：发送 / 接收批次', '数据：接口 / 报文 / 异常', '素材：截图、图表'],
    sections: [
      { title: '一、批次概述', ph: '说明接口范围、批次类型与执行时间。' },
      { title: '二、结果汇总', ph: '[ 数据表 · 发送或接收数量 / 接口覆盖 / 异常数量 ]', data: true },
      { title: '三、接口明细', ph: '[ 接口记录表 · 由所选批次数据自动填充 ]', data: true },
      { title: '四、异常与数据说明', ph: '呈现未发送数据、解析异常和代表性样本。' },
      { title: '五、结论与建议', ph: '汇总本次结果，并给出后续验证建议。' }
    ]
  },
  anomaly: {
    tone: 'anomaly',
    desc: '面向异常数据分析，呈现样本分布、原始数据和规则命中情况。',
    meta: ['适用：异常分析', '数据：异常接口 / 数据样本', '素材：原始报文、异常表'],
    sections: [
  ...
  archive: {
    tone: 'archive',
    desc: '面向正式交付，强化结果摘要、素材索引、版本与签署信息。',
    meta: ['适用：正式交付', '信息：签署人 / 版本号', '素材：附件目录、图表索引'],
    sections: [
      { title: '封面与签署页', ph: '项目名称、批次来源、报告生成者、版本与签署日期。' },
      { title: '一、数据来源说明', ph: '列出联试批次、接口范围与执行时间。' },
      { title: '二、结果汇总', ph: '[ 结果汇总表 · 指标 / 接口 / 异常 / 附件编号 ]', data: true },
      { title: '三、素材与附件索引', ph: '[table: 素材用途清单]，并回填图片、表格与日志附件位置。', media: true },
      { title: '四、交付结论', ph: '结果结论、遗留问题、后续责任人与跟踪节点。' }
    ]
  }
}
...
const templatePreview = computed(() => {
  ...
  if (name.includes('交付')) return PREVIEWS.archive
